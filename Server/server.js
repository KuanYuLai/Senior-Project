...
app.get('/job-history/:id', (req, res, next) => {
	...
	else if (id > job_history.length)
		res.status(400).send("Invalid id");
	else
		res.status(200).send(job_history[id-1]);
});
...
app.post('/new-job', (req, res, next) => {
  ...
  if (req.body) {
	...
	console.log("output data: " + JSON.stringify(output_data).replace(/\r?\n|\r/g, ''));
	...
	get_engine_data(output_data, async function(res_data) {
		...
		saved_data.jobTime = JSON.parse('"' + moment().format('MMMM DD YYYY, hh:mm') + '"');
